...
\begin{abstract}
In this chapter we show how to use \moments, a Python library for computing the
joint SFS and inferring demographic history, through its \demes interface. 
Following a discussion of SFS inference, we illustrate the general usage of 
\moments and sketch two specific examples. We conclude with a discussion of some 
additional considerations regarding the \moments library.
\end{abstract}
...
chromosome due to recombination (that is, by information stored in the Ancestral
...
code across software, ease of model implementation and reduction of errors.
...
which we try to reinfer using the both the original model and simpler misspecified 
ones. In the second example, we infer a relatively simple human-Neanderthal
...
describe data processing and inference setup, emphasizing the high-level
components of the analyses. For detailed information about each example,
...
\texttt{moments.Demes.Inference.uncerts(...)}, once a (locally) optimal model 
has been found. The following will emit a tab-separated file containing
parameter names, best-fit values and estimated standard errors.
\begin{minted}[gobble=4, frame=single]{python}
    log_file = 'log.txt'
...
were not genotyped in all four existing high-coverage archaic genome sequences, 
thatlacked high-confidence ancestral state assignments, or fell within 10 kb of 
exonic or promoter regions and were left with approximately 960 Mb of 
...
Throughout, we used $u=1.5\cdot 10^{-8}$ as an estimate of the mutation rate per 
generation and 29 years as the average generation time. We began by fitting 
...
resulting graph to get a two-population model that included continuous symmetric 
...
percent. We observe some large residuals in our best-fit model 
Figure~\ref{fig:humans}G-I), suggesting ...
...
\begin{figure}[t!]
    ...
    \caption{
      ...
      Marginal SFS fits between the model and empirical data are shown for MSL 
      (B) and GBR (C).
      Marginal two-way spectra predicted by the model and their residuals 
